...
setwd("C:\\Users\\yangd\\Desktop\\Bioinformatics\\lima-laa PgiC1")
report = as.data.frame(fread("PgiC1_demul.lima.report",stringsAsFactors=FALSE))

args = commandArgs(trailingOnly=TRUE)
if (length(args) == 0)
  stop("Provide (1) *.lima.report file, (2) optional output type png or png [def: png], and (3+) optional individual barcode (pair) names.\n", call.=FALSE)
reportPath = args[1]
output = "png"
if (length(args) >= 2)
  output = args[2]
